...
class ImageMatching:
    ...
    def __init__(self, image_paths, overlap_region=300):
        ...
        if overlap_region == -1:
            self.overlap_region = min(self.images[0].shape[1], self.images[1].shape[1])
        else:
    ...
    def find_most_common_shift(self):
        ...
        # Adjust pts1 back to the original coordinate system of img1
        ...
        # Adjust filtered_pts1 back to the original coordinate system of img1
    ...
    def stitch_images(self):
        ...
        # Calculate the shift using the coordinates of the matches
        shift_x = int((self.images[0].shape[1] - self.overlap_region) - x1 + x2)

        print(f"Stitching starts at img1: ({x1:.1f}, {y1:.1f}) and img2: ({x2:.1f}, {y2:.1f})")

        height1, width1 = self.images[0].shape
        height2, width2 = self.images[1].shape

        # Crop img1 to the left of the match point
        cropped_img1 = self.images[0][:, :int(x1)]
        # Crop img2 to the right of the match point
        ...
        # Place the cropped first image on the canvas
        stitched_image[:cropped_img1.shape[0], :cropped_img1.shape[1]] = cropped_img1

        # Place the cropped second image on the canvas, starting after the first image
        stitched_image[:cropped_img2.shape[0], cropped_img1.shape[1]:] = cropped_img2

        # Remove any potential excess black space
    ...
    def plot_feature_matching(self, do_print=True):
        ...
        img_matches = cv2.drawMatches(self.img1_overlap, self.kp1, self.img2_overlap, self.kp2, filtered_matches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
...
class StitchDirectory:
    ...
    def stitch_images(self):
        ...
        for next_image_path in self.image_paths[1:]:
            ...
            stitched_image = matcher.stitch_images()
            ...
            # Update current_image_path to the newly stitched image
